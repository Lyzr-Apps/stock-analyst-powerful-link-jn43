...
export async function updateScheduleMessage(
  ...
  if (!current.success || !current.schedule) {
    return { success: false, error: current.error || 'Schedule not found' }
  }
  const wasActive = current.schedule.is_active
  ...
  const created = await createSchedule({
    agent_id: current.schedule.agent_id,
    cron_expression: current.schedule.cron_expression,
    message: newMessage,
